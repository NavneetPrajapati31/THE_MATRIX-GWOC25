...
const OrderContainer = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await fetch("http://localhost:3001/orders-related/admin/orders");
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        try {
            const response = await fetch(`http://localhost:3001/orders-related/admin/orders/${orderId}`, {
                method: "PUT",
                headers: {
                ...
                <div key={order._id} className="order-card-yash">
                    ...
                    <div className="order-status">
                        ...
                        {order.orderStatus === "Pending" && (
                            <button className="status-btn confirm" onClick={() => updateOrderStatus(order._id, "Confirmed")}>
                                Confirm Order
                            </button>
                        )}

                        {order.orderStatus === "Confirmed" && (
                            <button className="status-btn pack" onClick={() => updateOrderStatus(order._id, "Packed")}>
                                Pack Order
                            </button>
                        )}

                        {order.orderStatus === "Packed" && (
                            <button className="status-btn ship" onClick={() => updateOrderStatus(order._id, "Shipped")}>
                                Ship Order
                            </button>
                        )}

                        {order.orderStatus === "Shipped" && (
                            <button className="status-btn deliver" onClick={() => updateOrderStatus(order._id, "Delivered")}>
                                Deliver Order
                            </button>
                        )}
